...
function encode_one {
  ...
  # Also check for season in the output path! If the disks aren't ripped into the typical "tv show" structure, but have
...
EOF
  ...
  if [ "$MODE" = FIRST_PASS ]; then
    ...
    x265_params="pools=$(nproc)"
    encoder_settings+=(-pix_fmt yuv420p10le -x265-params "$x265_params")
