...
PROFILE=$(./profile.sh "$FILE")
debug "Using profile $PROFILE"

source "data/profiles/$PROFILE"
[ -n "$CUT_STREAMS" ] || {
	debug "No CUT_STREAMS in profile $PROFILE"
...
function add_frames {
	...
	TIME=$(echo "$FULL_SECS + $f/23.98" | bc -l)
	...
	MODS=(${MODIFIERS[$I]})
		...
		# Name of the output for the latest link of the filter chain. Modifications might mutate this to add additional links.
	        # At the end, the value of this var is used as input to the concat filter for this stream.
                ...
                [ "$END_S" = "end" ] && {
       	                FILTER="$FILTER [$STREAM]${AORV}trim=start=$START_S,${AORV}setpts=PTS-STARTPTS[$OUTPUT];"
               	} || {
                       	FILTER="$FILTER [$STREAM]${AORV}trim=start=$START_S:end=$END_S,${AORV}setpts=PTS-STARTPTS[$OUTPUT];"
                }
		for M in "${MODS[@]}"; do
